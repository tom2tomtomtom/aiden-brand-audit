      ...
      {filteredBrands.map((brand) => (
        <div key={brand.name} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {brand.ads.slice(0, 12).map((ad) => (
              <div
                key={ad.adId}
                className="bg-black-card border-2 border-border-subtle hover:border-orange-accent transition-all"
              >
                {/* Ad image */}
                {ad.adImageUrl && (
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    ...
                    {ad.cta && (
                      <span className="px-2 py-0.5 bg-red-hot/10 border border-red-hot/30 text-[10px] text-red-hot uppercase tracking-wide">
                        {ad.cta}
                      </span>
                    )}
                  </div>
                  ...
                  {ad.adUrl && (
                    <a
                      ...
                      className="flex items-center gap-1 text-xs text-orange-accent hover:text-red-hot transition-colors"
                    >
